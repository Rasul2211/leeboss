...
console.log('\nWeight drives girth');
{
  ...
  const waistOf = (p: ReturnType<typeof buildProfile>) => p.torso[1]!.rx * p.torso[1]!.rz;
  const ratio = waistOf(heavy) / waistOf(light);
  check('same height, heavier body is thicker', ratio > 1.6, `waist area x${ratio.toFixed(2)}`);
  check(
    'height is unchanged by weight',
    near(light.heightM, heavy.heightM, 1e-9),
  );
}
...
console.log('\nBuild redistributes mass');
{
  ...
  const vRatio = (p: ReturnType<typeof buildProfile>) => p.torso[3]!.rx / p.torso[1]!.rx;
  check(
